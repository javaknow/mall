...
function load_cart_information(){
	$.getJSON(SITEURL+'/ajax/index.jhtml?act=cart&op=ajax_load&callback=?', function(result){
	    var obj = $('.head-user-menu .my-cart');
	    if(result){
	       	var html = '';
	       	if(result.cartItems.length >0){
	            for (var i = 0; i < cartItems.length; i++){
	                var goods = result.cartItems[i];
	            	html+='<dl id="cart_item_'+goods.id+'"><dt class="goods-name"><a href="'+goods.path+'">'+goods.name+'</a></dt>';
	            	html+='<dd class="goods-thumb"><a href="'+goods.path+'" title="'+goods.name+'"><img src="'+goods.image+'"></a></dd>';
		          	html+='<dd class="goods-sales"></dd>';
		          	html+='<dd class="goods-price"><em>&yen;'+goods.price+'×'+goods.quantity+'</dd>';
		          	html+='<dd class="handle"><a href="javascript:void(0);" onClick="drop_topcart_item('+goods.cart.id+','+goods.id+');">删除</a></dd>';
		          	html+="</dl>";
		        }
		        obj.find('.incart-goods').html(html);
    	        obj.find('.incart-goods-box').perfectScrollbar('destroy');
    	        obj.find('.incart-goods-box').perfectScrollbar();
	         	html = "共<i>"+result.quantity+"</i>种商品&nbsp;&nbsp;总计金额：<em>&yen;"+result.amountPaid+"</em>";
		        obj.find('.total-price').html(html);
		        if (obj.find('.addcart-goods-num').size()==0) {
		            obj.append('<div class="addcart-goods-num">0</div>');
		        }
		        obj.find('.addcart-goods-num').html(result.quantity);
				$('#rtoobar_cart_count').html(result.quantity).show();
	      } else {
...
function load_history_information(){
    $.getJSON(SITEURL+'/ajax/index.jhtml?act=index&op=viewed_info', function(result){
        var obj = $('.head-user-menu .my-mall');
        //会员登陆了
        if(result['member'] != undefined){
            if (typeof result['messageCount'] !== 'undefined') {
            	obj.find('#messageCount').html(result['messageCount']);
            }
            if (typeof result['couponCodeCount'] !== 'undefined') {
            	obj.find('#couponCodeCount').html(result['couponCodeCount']);
            }
        }
        var goods_id = 0;
        var text_append = '';
        ...
        if (typeof result['products'] !== 'undefined') {
            for (goods_id in result['products']) {
                var goods = result['products'][goods_id];
                text_append += '<li class="goods-thumb"><a href="'+goods['path']+'" title="'+goods['name']+
                '" target="_blank"><img src="'+goods['image']+'" alt="'+goods['name']+'"></a>';
                text_append += '</li>';
                n++;
                if (n > 4) {
                	break;
