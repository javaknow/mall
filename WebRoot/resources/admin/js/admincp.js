$(function() {
	...
	$("#trace_show").click(function() {
		$("div.bgSelector").toggle(300,function() {
			if ($(this).html() == '') {
				$(this).sColor({
					colors : bgColorSelectorColors, // 必填，所有颜色
					// c:色号（必填）
					// cName:颜色名称（可空）
					...
					defaultItem : ($.cookie('bgColorSelectorPosition') != null) ? $.cookie('bgColorSelectorPosition'): 22
					// 可选，第几个颜色的索引作为初始颜色，默认第1个颜色
				});
			}
		});// 切换显示
	});
	...
	$('div[data-param^="map-"]').find('i').click(function() {
		...
		if ($this.parent().hasClass('selected')) {
			$.getJSON('index.php?act=common&op=common_operations',{
				type : 'del',
				value : _value
			},
			function(data) {
	...
	/*
	 * if ($.cookie('commonPendingMatters') == null) { $.ajax({ url :
	 * 'index.php?act=common&op=ajax_pending_matters', async : false, success :
	 * function(data){ _commonPendingMatters = parseInt(data);
	 * $.cookie('commonPendingMatters', _commonPendingMatters, {expires :
	 * 1/24}); // 一小时更新一次 } }); } else { _commonPendingMatters =
	 * parseInt($.cookie('commonPendingMatters'));; }
	 */
