...
function get_list() {
    ...
    //查询产品
    $.getJSON(ApiUrl + "/product/list.jhtml?act=goods&op=goods_list" + window.location.search.replace("?", "&"), param,
    function(e) {
        if (!e) {
            e = [];
            e.datas = [];
...
function search_adv() {
    $.getJSON(ApiUrl + "/searchCondition.jhtml?act=index&op=search_adv",
    function(e) {
        var r = e.datas;
