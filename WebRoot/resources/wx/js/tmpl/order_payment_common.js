...
function toPay(a, e, p) {
	$.ajax({
		type : "post",
		url : ApiUrl + "/index.jhtml?act=" + e + "&op=" + p,
		data : {
			key : key,
			pay_sn : a
		},
										...
										if (s) {
											...
											$
													.ajax({
														type : "post",
														url : ApiUrl
																+ "/index.html?act=member_buy&op=check_pd_pwd",
														dataType : "json",
														data : {
															key : key,
															password : password
...
function goToPayment(a, e) {
	location.href = ApiUrl + "/index.jhtml?act=member_payment&op=" + e + "&key="
			+ key + "&pay_sn=" + a + "&password=" + password + "&rcb_pay="
			+ rcb_pay + "&pd_pay=" + pd_pay + "&payment_code=" + payment_code
}