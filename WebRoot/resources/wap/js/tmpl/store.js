...
$(function() {
	...
	$.ajax({
		type : "post",
		url : ApiUrl + "/store/info.jhtml?act=store&op=store_info",
		data : {
			key : t,
			store_id : e
		},
		...
		success : function(t) {
			...
			if (e.store_info.mb_title_img) {
				$(".store-top-bg .img").css("background-image",
						"url(" + e.store_info.mb_title_img + ")")
			} else {
				var a = [];
	...
	$("#goods_rank_tab").find("a").click(function() {
		...
		$.ajax({
			...
			success : function(e) {
				if (e.code == 200) {
					var o = template.render("goodsrank_" + t + "_tpl",
							e.datas);
					$("#goodsrank_" + t).html(o)
				}
			}
	...
	$("#store_notcollect").live("click",function() {
		...
		if (t) {
			...
			$("#store_collected").show();
			var o;
			var s = (o = parseInt($("#store_favornum_hide")
					.val())) > 0 ? o + 1 : 1;
