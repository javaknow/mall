...
$(function() {
	...
	function t() {
		...
		$.ajax({
			type : "post",
			url : ApiUrl + "/order/list.jhtml?act=member_order&op=order_list&page=" + page + "&curpage=" + curpage,
			data : {
				key : e,
				state_type : t,
				order_key : r
...
//checkLogin(e.login);
	...
	function i(r) {
		$.ajax({
			type : "post",
			url : ApiUrl + "/order/delete.jhtml?act=member_order&op=order_delete",
			data : {
				order_id : r,
				key : e
			},
	...
	function l() {
		var e = $(this).attr("order_id");
		location.href = WapSiteUrl + "/member/order/evaluation.jhtml?sn=" + e
	}
	
	//订单的再次评价
	function d() {
		var e = $(this).attr("order_id");
		location.href = WapSiteUrl + "/member/order/evaluation_again.jhtml?sn=" + e
	}
	
	//订单的物流查询
	function c() {
		var e = $(this).attr("order_id");
		location.href = WapSiteUrl + "/member/order/delivery.jhtml?sn=" + e
	}
...
function get_footer() {
	if (!footer) {
		footer = true;
		$.ajax({
			url : WapSiteUrl1 + "/resources/wap1/js/tmpl/footer.js",
			dataType : "script"
		})
	}
}