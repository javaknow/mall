...
$(function (){
  ...
  function get_detail(goods_id) {
      //渲染页面
      $.ajax({
         url:ApiUrl+"/product/goods_detail.jhtml?act=goods&op=goods_detail",
         type:"get",
            ...
            if(!data.error){
              ...
              if (data.goods_info.is_virtual == '1') {
                  $("#buy-now").click(function() {
                      var key = getCookie('username');//登录标记
                      if (!key) {
  ...
  $('body').on('click', '#goodsBody,#goodsBody1', function(){
      window.location.href = WapSiteUrl+'/product/info.jhtml?goods_id=' + goods_id;
  });
  $('body').on('click', '#goodsEvaluation,#goodsEvaluation1', function(){
      window.location.href = WapSiteUrl+'/product/eval_list.jhtml?goods_id=' + goods_id;
  });
