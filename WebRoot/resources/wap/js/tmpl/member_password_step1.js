$(function() {
	...
	$("#send").click(function() {
		if ($.sValid()) {
			...
			$.ajax({
				type : "post",
				url : ApiUrl + "/modify_password_step2.jhtml?act=member_account&op=modify_password_step2",
				data : {
					key : e,
					captcha : a,
					captchaId : t
	...
	$("#nextform").click(function() {
		...
		if (a) {
			$.ajax({
				type : "post",
				url : ApiUrl+ "/modify_password_step3.jhtml?act=member_account&op=modify_password_step3",
				data : {
					key : e,
					auth_code : a
				},
				...
				success : function(e) {
					if (e.code == 200&&e.message.type=="success") {
						...
						setTimeout("location.href = WapSiteUrl+'/member/member_password_step2.jhtml'",1e3)
					} else {
						errorTipsShow("<p>" + e.message.content + "</p>")
						$("#codeimage").attr("src",ApiUrl+ "/makecode.jhtml?act=seccode&op=makecode&captchaId="+ $("#codekey").val()+ "&t="+ Math.random());
						$("#captcha").val("")
