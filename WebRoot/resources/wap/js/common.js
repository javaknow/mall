...
function buildUrl(type, data) {
    switch (type) {
        case 'keyword':
            return WapSiteUrl + '/product/list.jhtml?keyword=' + encodeURIComponent(data);
        case 'special':
            return WapSiteUrl + '/special.html?special_id=' + data;
        case 'goods':
            return WapSiteUrl + '/product/detail.jhtml?goods_id=' + data;
        case 'url':
            return data;
    }
    return WapSiteUrl;
...
function getSearchName() {
	...
	if (keyword == '') {
	    if(getCookie('deft_key_value') == null||getCookie('deft_key_value') == '') {
	    	//ApiUrl = "http://b2b2c.shopnctest.com/dema/mo_bile";
	        $.getJSON(ApiUrl + '/search_hot_info/index.jhtml?act=index&op=search_hot_info', function(result) {
	        	var data = result.datas;
	        	console.info("===="+data);
	        	if(typeof data != 'undefined') {
	            	$('#keyword').attr('placeholder',data);
	            	$('#keyword').html(data);
	            	addCookie('deft_key_name',data,1);
	            	//addCookie('deft_key_value',data.value,1);
	            	addCookie('deft_key_value',data,1);
	        	} else {
     ...
     */
        areaSelected: function(options) {
            ...
            function _getAreaList() {
                $.ajax({//获取区域列表
                    type:'get',
                    url:ApiUrl+'/area/area_list.jhtml?act=area&op=area_list',
                    data:{area_id:ASID},
                    dataType:'json',
                    async:false,
                    success:function(result){
                        if (result.area_list.length == 0) {
                            _finish();
                            return false;
                        }
                        if (ASINIT) {
                        ...
                        for(var i=0;i<data.area_list.length;i++){
                            area_li += '<li><a href="javascript:void(0);" data-id="' + data.area_list[i].id + '" data-name="' + data.area_list[i].name + '"><h4>' + data.area_list[i].name + '</h4><span class="arrow-r"></span> </a></li>';
                        }
                        $('#areaSelected').find(".nctouch-default-list").html(area_li);
                        if (typeof(myScrollArea) == 'undefined') {
                            if (typeof(IScroll) == 'undefined') {
                                $.ajax({
                                    url: WapSiteUrl1+'/resources/wap1/js/iscroll.js',
                                    dataType: "script",
                                    async: false
                                  });
                            }
...
function loadSeccode(){
    ...
    $.ajax({
        type:'get',
        url:ApiUrl+"/makecodekey.jhtml?act=seccode&op=makecodekey",
        async : false,
        dataType: 'json',
        success:function(result){
            $("#codekey").val(result.captchaId);
        }
    });
