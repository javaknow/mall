...
public class RegisterController extends BaseController {
	...
	public @ResponseBody
	boolean checkParent(String parentNumber) {
		if (StringUtils.isEmpty(parentNumber)) {
			return false;
		}
		if (!memberService.usernameDisabled(parentNumber) && memberService.usernameExists(parentNumber)) {
			return true;
		} else {
			return false;
		}
	...
	public @ResponseBody
	Message submit(String captchaId, String captcha, String username,String parentNumber, String email, HttpServletRequest request, HttpServletResponse response, HttpSession session) {
		...
		Member parent = memberService.findByUsername(parentNumber);
		if(parent==null){
			return Message.error("推荐人不存在！");
		}
		
		member.setNumber(memberService.setNumber(setting.getMemberNumberPrefix(),setting.getMemberNumberLength()));
	...
	public String agreement(HttpServletRequest request, HttpServletResponse response, ModelMap model) {
		return "/shop/register/agreement";
	}
