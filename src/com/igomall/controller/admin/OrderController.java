...
import com.igomall.entity.ShippingItem;
...
public class OrderController extends BaseController {
	...
	public String complete(Long id, RedirectAttributes redirectAttributes,HttpServletRequest reqeust) {
		Order order = orderService.find(id);
		Admin admin = adminService.getCurrent();
		if (order != null && !order.isExpired() && order.getOrderStatus() == OrderStatus.confirmed && !order.isLocked(admin)) {
			...
			if(order.getPaymentStatus()==PaymentStatus.refunded||order.getPaymentStatus()==PaymentStatus.partialPayment||order.getShippingStatus()==ShippingStatus.returned||order.getShippingStatus()==ShippingStatus.partialReturns){
				
			}else{
				bonusCouponService.create(order, BonusCoupon.Type.member,reqeust);
			}
