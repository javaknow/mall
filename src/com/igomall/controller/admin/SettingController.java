...
import com.igomall.Setting.ConsultationAuthority;
import com.igomall.Setting.ReviewAuthority;
import com.igomall.Setting.RoundType;
import com.igomall.Setting.SignInType;
import com.igomall.Setting.StockAllocationTime;
import com.igomall.Setting.WatermarkPosition;
...
public class SettingController extends BaseController {
	...
	public String edit(ModelMap model) {
		model.addAttribute("signInTypes", SignInType.values());
	...
	public String update(Setting setting, MultipartFile watermarkImageFile, RedirectAttributes redirectAttributes) {
		...
		try {
			org.springframework.core.io.Resource resource = new ClassPathResource(CommonAttributes.LX_PROPERTIES_PATH);
			...
			if (setting.getIsDevelopmentEnabled()) {
				if (!templateUpdateDelay.equals("0") || !messageCacheSeconds.equals("0")) {
					...
					properties.store(outputStream, "igomall PROPERTIES");
				}
			} else {
				if (templateUpdateDelay.equals("0") || messageCacheSeconds.equals("0")) {
					outputStream = new FileOutputStream(resource.getFile());
