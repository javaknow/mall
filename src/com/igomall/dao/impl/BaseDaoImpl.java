...
import com.igomall.util.JdbcUtils;
...
public abstract class BaseDaoImpl<T, ID extends Serializable> implements BaseDao<T, ID> {
	...
	public List<T> findListJdbc(String sql) {
	...
	public Page<T> findPageJdbc(String sql,String sql1,Pageable pageable) {
		long total = count(sql1);
		int totalPages = (int) Math.ceil((double) total / (double) pageable.getPageSize());
		if (totalPages < pageable.getPageNumber()) {
			pageable.setPageNumber(totalPages);
		}
		...
		QueryRunner qr = new QueryRunner(JdbcUtils.getDataSource());
		try {
			result = (List<T>) qr.query(sql, new BeanListHandler<T>(entityClass));
		} catch (SQLException e) {
