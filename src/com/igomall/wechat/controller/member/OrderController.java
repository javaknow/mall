...
@Controller("wechatMemberOrderController")
...
public class OrderController extends BaseController {
	...
	public String info(ModelMap model) {
		...
		if (cart == null || cart.isEmpty()) {
			return "redirect:/wechat/cart/list.jhtml";
		}
		if (!isValid(cart)) {
			return ERROR_VIEW;
		}
		...
		return "/wechat/member/order/info";
	}
	...
	public @ResponseBody
	Message create(String cartToken, Long receiverId, Long paymentMethodId,
	...
	public String payment(String sn, ModelMap model) {
		...
		model.addAttribute("order", order);
		return "/wechat/member/order/payment";
	}
	...
	public String list5(ModelMap model) {
		...
		model.addAttribute("orders",
				orderService.findList(member, null, null, null));
	...
	public String view(String sn, ModelMap model) {
		...
		model.addAttribute("order", order);
		return "/wechat/member/order/view";
	}
	...
	Message refunds(String sn) {
		...
		refunds.setOperator("");
		/*
		 * if (!isValid(refunds)) { return ERROR_MESSAGE; }
		 */
